...
parser.add_argument('input', help='script file', nargs='?')
...
opcodeslen = {
    0x00: 1, # Handle null bytes/padding
    0x01: 5,
    0x02: 5,
    0x03: 5,
    0x04: 5,
    ...
    0x88: 1,
    0x8B: 5,
    0x8C: getlen_opcode_4_plus_sz, # Text opcode
    0x8D: 1,
    0xD8: getlen_opcode_4_plus_sz, # Text opcode
}
...
if __name__ == "__main__":
    ...
    output_dir = "scripts/final-complete"
    os.makedirs(output_dir, exist_ok=True)
    
    # If input file is specified, process just that file
    if args.input:
        script_files = [args.input]
    else:
        # Process all .opcodescript files in the scripts directory
        script_dir = "raw/final-complete/scripts"
        script_files = glob.glob(os.path.join(script_dir, "*.opcodescript"))
    
    for script_file in script_files:
        print(f"\nProcessing {script_file}...")
        args.input = script_file
        
        # Get the base filename without path
        base_filename = os.path.basename(script_file)
        outputpath = os.path.join(output_dir, base_filename + '-extr.txt')
        
        # Reset opcodeslen to default for each file
        opcodeslen = {
            0x00: 1, # Handle null bytes/padding
            0x01: 5,
            0x02: 5,
        ...
        if os.path.getsize(outputpath) == 0:
            ...
            print(f"Removed empty output file: {outputpath}")
        else:
            print(f"Created output file: {outputpath}")
